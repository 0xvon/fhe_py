...
import util.math.number_theory as nbtheory
from util.math.bit_operations import bit_reverse_vec, reverse_bits
...
class NTTContext:
    ...
    def ntt(self, coeffs, rou):
        ...
        assert len(rou) == num_coeffs, \
            "Length of the roots of unity is too small. Length is " + len(rou) # type: ignore
    ...
    def ftt_fwd(self, coeffs):
        ...
        """
        num_coeffs = len(coeffs)
        assert num_coeffs == self.degree, f"ftt_fwd: input length {len(coeffs)} does not match context degree {self.degree}"
...
class FFTContext:
    ...
    def precompute_fft(self):
        ...
        for i in range(self.fft_length):
            ...
            self.roots_of_unity[i] = complex(cos(angle), sin(angle)) # type: ignore
            self.roots_of_unity_inv[i] = complex(cos(-angle), sin(-angle)) # type: ignore
    ...
    def fft_inv(self, coeffs):
        ...
        for i in range(num_coeffs):
            result[i] /= num_coeffs # type: ignore
    ...
    def embedding_inv(self, coeffs):
        ...
        for i in range(num_coeffs):
            to_scale_down[i] /= num_coeffs # type: ignore
